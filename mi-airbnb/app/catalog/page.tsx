      ...
      <header className="sticky top-0 z-20 border-b border-zinc-200 bg-[#f7f7f7]">
        <div className="mx-auto flex max-w-[1700px] items-center justify-between gap-5 px-4 py-4 md:px-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              type="button"
              className="grid h-12 w-12 place-items-center rounded-full bg-zinc-200 text-[18px]"
              aria-label="Idioma"
            >
              🌐
            </button>
        ...
        <section className="grid gap-6 md:grid-cols-2 md:h-[calc(100vh-186px)]">
          <div className="space-y-6 md:overflow-y-auto md:pr-3">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <h1 className="text-[43px] leading-[1.05] font-extrabold text-zinc-900">
                Mas de {sortedRooms.length} alojamientos en San Carlos de Bariloche
              </h1>

              <div className="inline-flex items-center gap-2 rounded-full bg-zinc-100 px-3 py-2 text-[15px] font-semibold text-zinc-700">
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
              {sortedRooms.map((room) => (
                <CatalogResultCard key={room.id} room={room} />
              ))}
            </div>
          </div>
